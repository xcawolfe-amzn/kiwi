...
class TestXMLState:
    ...
    def test_get_ec2_part_layout(self):
        ...
        xml_data = description.load()
        state = XMLState(xml_data)
        assert state.get_ec2_part_layout() is True

    def test_get_ec2_part_layout_attribute_missing(self):
        """Test EC2 layout backward compatibility when attribute missing"""
        # Simulate older XML files without ec2_layout attribute
        # by mocking the build_type.get_ec2_part_layout() method to raise AttributeError
        mock_build_type = Mock()
        mock_build_type.get_ec2_part_layout.side_effect = AttributeError(
            'get_ec2_part_layout'
        )
        self.state.build_type = mock_build_type
        assert self.state.get_ec2_part_layout() is False
